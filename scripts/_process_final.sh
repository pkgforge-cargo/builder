...
#slow on purpose to avoid rate limits, 10,000 packages take ~ 5 hrs to process.
...
pushd "$(mktemp -d)" &>/dev/null
 ...
 mkdir -p "${OUT_DIR}" "${TEMP_DIR}" "${WORK_DIR}"
...
#Processor func
 process_crate_binary()
 {
   ...
   #Process  
    echo -e "Processing: $crate_name ($crate_version)"
             ...
             if [[ -f "Cargo.toml" ]]; then
                 ...
                 if ! cargo metadata --format-version 1 --no-deps 2>/dev/null | grep -m 1 -qoiE '"kind"[[:space:]]*:[[:space:]]*\[[^]]*"lib"[^]]*\]'; then
                    #Save
                     ...
                     echo "✓ Binary crate: $crate_name"
                     popd &>/dev/null
                     rm -rf "$work_dir"
                     return 0
                 else
                    #Skip
                     echo "⚬ Library crate (skipped): $crate_name"
                     popd &>/dev/null
                     rm -rf "$work_dir"
                     return 0
                 fi
             else
                 echo "✗ No Cargo.toml found: $crate_name"
             fi
         else
             echo "✗ Failed to extract: $crate_name"
         fi
      else
          echo "✗ Download failed: $crate_name (attempt $((retries + 1)))"
      fi
     #Cleanup 
      popd &>/dev/null
      rm -rf "$work_dir"
   ...
   #Fail if exceeded  
     echo "✗ Failed after $max_retries retries: $crate_name"
     return 1
 }
 export -f process_crate_binary
 export OUT_DIR TEMP_DIR WORK_DIR
#Process
 echo -e "\n[+] Processing Crates [$(wc -l < "${WORK_DIR}/crates_to_process.txt")] ...\n"
...
#Print stats
 du -bh "${OUT_DIR}/CRATES_INPUT.json"
